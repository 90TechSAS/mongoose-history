...
module.exports = function historyPlugin(schema, options) {
  ...
  // Listen on update
  // Log the entire _update object because it can be something else than a $set
  schema.pre('update', function(next) {
    // Remove $ in the keys, since mongo won't allow it
    let d = Object.keys(this._update).reduce(function(acc, k){
      acc['_' + k.slice(1)] = this._update[k]
      return acc
    }, {});
